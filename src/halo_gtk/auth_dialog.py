...
class AuthDialog(Adw.Dialog):
    ...
    def _authenticate(self, email: str, password: str, otp_code: str | None) -> None:
        try:
            ...
            try:
                save_account_email(email, expected_client=client)
            except SessionSupersededError:
                raise
            except Exception as exc:
                _log.warning("Failed to save Ring account email: %s", exc)
            if not start_client(client):
            ...
            from halo_gtk.ring_client import is_ring_session_rejected

            if isinstance(exc, Requires2FAError):
                GLib.idle_add(self._show_otp_prompt)
            elif is_ring_session_rejected(exc):
                GLib.idle_add(
                    self._show_error,
                    "Ring accepted sign-in, but rejected the API session. Disable VPN"
                    " or exclude Ring traffic, then retry.",
                )
    ...
    def _on_auth_success(self) -> bool:
        self.close()
        # Refresh the main window specifically — sign-in can be launched from the
        # Settings window, and get_active_window() would refresh that instead,
        # leaving the main window's banner and device list stale.
